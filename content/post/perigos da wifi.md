...
Em meados de 2019 ou 2020 eu percebi/descobri um padrão nas credenciais que vinham com o modem roteador de uma operadora grande e isso me fez criar um script para testar minha teoria. Assim, sempre que alguém utilizava uma rede com as credenciais padrões desta operadora, eu testava força bruta com a wordlist que meu script me retornava e sempre era certeira para achar a credencial da rede. Em outras palavras, quando a análise combinatória é feita e a wordlist gerada de acordo com o padrão que eu detectei, entre os resultados gerados sempre estaria a senha correta da rede e isso acarretava em um ataque eficaz, bastando apenas testar todas as possibilidades (senhas do arquivo) e a senha seria quebrada e descoberta. Fui publicar com finalidade acadêmica e meu script precisou ser tirado do ar pós recebimento de uma notificação para retirada do repositório do github - eu também acabei vacilando por ter deixado o nome da empresa. Hoje ela não utiliza mais o mesmo padrão, mas ainda temos muitos usuários que estão utilizando esta configuração antiga e minha vizinha é um exemplo - Até por isso que precisei retirar meu script do ar (farei um post e um vídeo específico sobre isso futuramente).
